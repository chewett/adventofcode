 ...
 * in the middle of the second addx -1, so the value of register X is the starting value, 1, plus all of the other
...
public class Day10 {
    ...
    /**
     * Run the program and work out the signal strength at each of the named cycles (20, 60, 100, 140, 180, 220)
     * @param programData Program data to run
     * @return Sum of the six signal strengths
     */
    public long solvePartOne(List<String> programData) {
        int register = 1;
        int cycle = 1;
        int signalStrength = 0;

        for(String line : programData) {
            //Does nothing but increment the cycles
            if(line.equals("noop")) {
                cycle++;
            }else{
                String[] part = line.split(" ");
                int addValue = Integer.parseInt(part[1]);

                //Handle the fact this takes two cycles, so add one then do the check
                cycle +=1;
            ...
            //Do the check again to see if after a noop or second part of the add it has reached the right number
            if(cycle == 20 || cycle == 60 || cycle == 100 || cycle == 140 || cycle == 180 || cycle == 220) {
                signalStrength += (cycle * register);
            }
        }
    ...
    /**
     * Run the program and work out the image generated on the CRT
     * @param programData Program data to run
     * @return The number of lit pixels (but also output the CRT screen during running this)
     */
    public long solvePartTwo(List<String> programData) {
        ...
        for(String line : programData) {
            if(line.equals("noop")) {
                ...
                cycle +=1;
                int xPos = cycle % 40;
                int yPos = Math.floorDiv(cycle, 40);
                //This is the "half instruction code" as above
                //Draw a pixel if the selection region is the same as the register
                if(register -1 == xPos || register == xPos || register+1 == xPos) {
                    grid.setValueAtPosition(xPos, yPos, '#');
                }
            ...
            //Again same as above to handle the "after instruction code"
            int xPos = cycle % 40;
            int yPos = Math.floorDiv(cycle, 40);
            if(register -1 == xPos || register == xPos || register+1 == xPos) {
                grid.setValueAtPosition(xPos, yPos, '#');
        ...
        //Print it out so I can see the letters, in the future I will code something to format their letters
        grid.print();
    ...
    public static void main(String[] args) {
        ...
        System.out.println("The sum of the signal strengths is " + partOne);

        System.out.println("The dot matrix display is:");
        long partTwo = d.solvePartTwo(programData);
        System.out.println("and the number of lit pixels is "+ partTwo);
