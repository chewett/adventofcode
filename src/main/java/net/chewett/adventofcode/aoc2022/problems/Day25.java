 ...
 * (2 times 625) plus (-2 times 125) plus (-1 times 25) plus (0 times 5) plus (1 times 1). That's 1250 plus -250 plus
 * -25 plus 0 plus 1. 976!"
 *
...
public class Day25 {
    ...
    /**
     * Convert a number into a SNAFU number
     * @param n Standard base 10 number
     * @return String representing a SNAFU number
     */
    public String convertToSnafuNumber(long n) {
        ...
        for(int charIndex = baseFive.length() - 1; charIndex >= 0; charIndex--) {
            ...
            int thisVal = Integer.parseInt(""+baseFive.charAt(charIndex));
            thisVal += carryOver;
            //If the value is 3 or 4 then we can't represent this so need to carry this over
            if(thisVal >= 3) {
                carryOver = 1;
                thisVal -= 5;
            }else{
                //Make sure you reset the carry every time
                carryOver = 0;
            }
    ...
    /**
     * Convert from a SNAFU number into a more standard base 10 number
     * @param str String representing the SNAFU number
     * @return Base 10 number converted from SNAFU
     */
    private long convertFromSnafuNumber(String str) {
        long thisNumber = 0;
        long multiplier = 1;
    ...
    /**
     * Converts all the SNAFU numbers to base 10, then add them up, then convert them back
     * @param numbers List of SNAFU numbers
     * @return Sum of all the SNAFU numbers as a SNAFU number
     */
    public String solvePartOne(List<String> numbers) {
        long total = 0;
        for(String number : numbers) {
            total += this.convertFromSnafuNumber(number);
        }
        return this.convertToSnafuNumber(total);
    }


    public static void main(String[] args) {
        List<String> numbers = ProblemLoader.loadProblemIntoStringArray(2022, 25);

        Day25 d = new Day25();
        String partOne = d.solvePartOne(numbers);
        System.out.println("The SNAFU number to enter into Bob " + partOne);
    }
