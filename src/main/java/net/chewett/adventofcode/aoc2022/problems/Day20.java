...
import net.chewett.adventofcode.datastructures.CustomLinkedList;
...
public class Day20 {
    ...
    /**
     * Attempts to solve the Elves decryption by finding the grove coordinates
     * @param input List of Integer inputs that are encrypting the data
     * @return Sum of the three numbers that form the grove coordinates
     */
    public long solveElvesDecryption(List<Integer> input) {
        return this.solveElvesDecryption(input, 1, 1);
    }

    /**
     * Attempts to solve the Elves decryption by finding the grove coordinates
     * @param input List of Integer inputs that are encrypting the data
     * @param numberOfTimesToMix The number of times to mix the inputs
     * @param multiplierFactor Factor to use as multiplication of the input
     * @return Sum of the three numbers that form the grove coordinates
     */
    public long solveElvesDecryption(List<Integer> input, int numberOfTimesToMix, long multiplierFactor) {
        CustomLinkedList<Long> head = null;
        CustomLinkedList<Long> tail = null;
        CustomLinkedList<Long> zeroPoint = null;

        List<CustomLinkedList<Long>> originalOrder = new ArrayList<>();

        //Start creating the list
        for(int i : input) {
            long realI = i * multiplierFactor;

            CustomLinkedList<Long> thisElement = new CustomLinkedList<>(realI);
            originalOrder.add(thisElement);
            ...
            if(head == null) {
                ...
                //Otherwise each new element is the new tail
                CustomLinkedList<Long> oldTail = tail;
                tail = thisElement;
                tail.setPrev(oldTail, true);
            }

            //Keep track of the zero element
            if(realI == 0) {
                zeroPoint = thisElement;
            }
        }

        //Close the tail so its a circular loop
        head.setPrev(tail, true);

        //Mix it a number of times
        for(int mixNumber = 0; mixNumber < numberOfTimesToMix; mixNumber++) {
            for (CustomLinkedList<Long> valToMove : originalOrder) {
                //Now this is the tricky bit! Its modulo - 1 because you are "removing" this element, then shifting
                //So you need to modulo -1 to make it work
                long valueToMove = valToMove.getVal() % (input.size() - 1);

                CustomLinkedList<Long> beforeVal = valToMove.getPrev();
                CustomLinkedList<Long> afterVal = valToMove.getNext();

                //If its zero, don't move at all!
                if (valueToMove != 0) {
                    //Seal the list so we have removed this element
                    afterVal.setPrev(beforeVal, true);

                    //Move it forward
                    if (valueToMove > 0) {
                        CustomLinkedList<Long> nextToInsert = afterVal;
                        for (long moveIndex = 0; moveIndex < valueToMove; moveIndex++) {
                            nextToInsert = nextToInsert.getNext();
                        }

                        CustomLinkedList<Long> newPrev = nextToInsert.getPrev();
                        nextToInsert.setPrev(valToMove, true);
                        newPrev.setNext(valToMove, true);

                        //Move it backward
                    } else if (valueToMove < 0) {
                        CustomLinkedList<Long> prevToInsert = beforeVal;
                        for (long moveIndex = -1; moveIndex > valueToMove; moveIndex--) {
                            prevToInsert = prevToInsert.getPrev();
                        }

                        CustomLinkedList<Long> newNext = prevToInsert.getPrev();
                        valToMove.setNext(prevToInsert, true);
                        valToMove.setPrev(newNext, true);
                    }
                }
            }
        }

        //Now work out the grove coordinate values
        long sumOfNumber = 0;
        CustomLinkedList<Long> current = zeroPoint;
        for(int i = 1; i <= 3000; i++) {
            current = current.getNext();
            if(i % 1000 == 0) {
                sumOfNumber += current.getVal();
            }
        }
    ...
    /**
     * Runs the algorithm once to get the grove coordinate
     * @param input Encrypted input
     * @return Sum of the grove coordinates
     */
    public long solvePartOne(List<Integer> input) {
        return this.solveElvesDecryption(input);
    }

    /**
     * Runs the algorithm 10 times and properly with the decryption multiplier
     * @param input Encrypted input
     * @return Sum of the grove coordinates
     */
    public long solvePartTwo(List<Integer> input) {
        return this.solveElvesDecryption(input, 10, 811589153L);
    }

    public static void main(String[] args) {
        List<Integer> input = ProblemLoader.loadProblemIntoIntegerList(2022, 20);

        Day20 d = new Day20();
        long partOne = d.solvePartOne(input);
        System.out.println("The sum of the grove coordinates is " + partOne);

        long partTwo = d.solvePartTwo(input);
        System.out.println("The sum of the grove coordinates with the right decryption key is " + partTwo);
