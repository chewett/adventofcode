...
public class Day1 {
    ...
    /**
     * Find the elf carrying the most calories and count the calories they are holding
     * @param foodstuffs List of calories each elf carries
     * @return The number of calories held by the elf holding the most
     */
    public long solvePartOne(List<String> foodstuffs) {
        int maxFound = 0;
        int currentFound = 0;

        for (String str : foodstuffs) {
            if (!str.equals("")) {
                currentFound += Integer.parseInt(str);

            } else {
                //Start counting the next elf on the empty string
                maxFound = Math.max(maxFound, currentFound);
                currentFound = 0;
            }
        }

        //Handle the final elves total
        maxFound = Math.max(maxFound, currentFound);
    ...
    /**
     * This time get the colories carried by all the elves and sum up the highest three calorie count.
     * @param foodstuffs List of calories each elf carries
     * @return The sum of the three highest calorie count
     */
    public long solvePartTwo(List<String> foodstuffs) {
        int currentFound = 0;
        ...
        for (String str : foodstuffs) {
            if (!str.equals("")) {
                ...
                allFound.add(currentFound);
                currentFound = 0;
            }
        }
        allFound.add(currentFound);

        //Reverse sort and then return the top three elements
        allFound.sort(Collections.reverseOrder());
        return allFound.get(0) + allFound.get(1) + allFound.get(2);
    ...
    public static void main(String[] args) {
        ...
        Day1 d = new Day1();
        long partOne = d.solvePartOne(foodstuffs);
        System.out.println("The elf with the most food holds " + partOne);

        long partTwo = d.solvePartTwo(foodstuffs);
        System.out.println("The sum of the foodstuffs held by the top three elves is " + partTwo);
