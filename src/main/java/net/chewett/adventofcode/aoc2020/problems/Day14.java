...
public class Day14 {
    ...
    /**
     * Simplem helper function to be used in both parts to convert a long to a bit pattern
     * @param value Long value to convert into a bit pattern
     * @return The string representing this number as a bit pattern
     */
    private String getValueAsBitString(long value) {
        StringBuilder valueToModifyAsBitsBuilder = new StringBuilder();
        long currentDivisor = (long)Math.pow(2, 35);
        while(currentDivisor > 0) {
            long howManyOfDivisor = Math.floorDiv(value, currentDivisor);
            value = value % currentDivisor;

            if(howManyOfDivisor > 0) {
                valueToModifyAsBitsBuilder.append("1");
            }else{
                valueToModifyAsBitsBuilder.append("0");
            }

            currentDivisor = Math.floorDiv(currentDivisor, 2);
        }

        return valueToModifyAsBitsBuilder.toString();
    }

    /**
     * After executing the initialization program this will return the sum of all values left in memory
     * @param program The given initialization program
     * @return The value of the sum of all values left in memory
     */
    public long solvePartOne(List<String> program) {
        HashMap<Long, Long> memory = new HashMap<>();

        String currentMask = "";
        for(String instruction : program) {
            String[] instrParts = instruction.split(" = ");
            //If this instruction is a mask, save the new mask
            if(instrParts[0].equals("mask")) {
                currentMask = instrParts[1];

            }else{
                //Otherwise, time to process some data!
                long valueToModify = Integer.parseInt(instrParts[1]);
                long memoryAddressToWrite = Integer.parseInt(instrParts[0].substring(4, instrParts[0].length() - 1));
                String valueToModifyAsBits = this.getValueAsBitString(valueToModify);

                //Now start creating the bit string we will want to write into memory
                StringBuilder realValueToWriteBuilder = new StringBuilder();
                for(int stringIndex = 0; stringIndex < valueToModifyAsBits.length(); stringIndex++) {
                    if(currentMask.charAt(stringIndex) == '1') {
                        realValueToWriteBuilder.append("1");
                    }else if(currentMask.charAt(stringIndex) == '0') {
                        realValueToWriteBuilder.append("0");
                    }else{
                        realValueToWriteBuilder.append(valueToModifyAsBits.charAt(stringIndex));
                    }
                }

                memory.put(memoryAddressToWrite, Long.parseLong(realValueToWriteBuilder.toString(), 2));
            }
        }

        long totalSum = 0;
        for(Map.Entry<Long, Long> e : memory.entrySet()) {
            totalSum += e.getValue();
        }
        return totalSum;
    ...
    /**
     * After executing the initialization program with the V2 decoder this will return the sum of all values left in memory
     * @param program The given initialization program
     * @return The value of the sum of all values left in memory
     */
    public long solvePartTwo(List<String> program) {
        HashMap<Long, Long> memory = new HashMap<>();
        ...
        for(String instruction : program) {
            ...
            if(instrParts[0].equals("mask")) {
                ...
                //Otherwise, time to process some data!
                long valueToWrite = Integer.parseInt(instrParts[1]);
                long memoryAddressToWrite = Integer.parseInt(instrParts[0].substring(4, instrParts[0].length() - 1));
                String valueToModifyAsBits = this.getValueAsBitString(memoryAddressToWrite);

                //Now start working out the location we want to write the value into memory
                StringBuilder realLocsToWriteBuilder = new StringBuilder();
                for(int stringIndex = 0; stringIndex < valueToModifyAsBits.length(); stringIndex++) {
                    if(currentMask.charAt(stringIndex) == '1') {
                        realLocsToWriteBuilder.append("1");
                    }else if(currentMask.charAt(stringIndex) == '0') {
                        realLocsToWriteBuilder.append(valueToModifyAsBits.charAt(stringIndex));
                    }else{
                        realLocsToWriteBuilder.append(currentMask.charAt(stringIndex));
                    }
                }

                String realLocsToWrite = realLocsToWriteBuilder.toString();
                List<String> allLocsToWrite = new ArrayList<>();
                allLocsToWrite.add("");
                //Slowly build up every location we want to write into memory
                for(int locIndex = 0; locIndex < realLocsToWrite.length(); locIndex++) {
                    List<String> newLocs = new ArrayList<>();
                    for(String location : allLocsToWrite) {
                        //If there is an X, we need to write both possibilities in
                        if(realLocsToWrite.charAt(locIndex) == 'X') {
                            newLocs.add(location + "0");
                            newLocs.add(location + "1");
                        }else{
                            newLocs.add(location + realLocsToWrite.charAt(locIndex));
                        }
                    }
                    allLocsToWrite = newLocs;
                }

                //Now we have every string we want to write, lets start actually writing the values
                for(String locationToWrite : allLocsToWrite) {
                    memory.put(Long.parseLong(locationToWrite, 2), valueToWrite);
                }
            }
        }
    ...
    public static void main(String[] args) {
        Day14 d = new Day14();
        List<String> program = ProblemLoader.loadProblemIntoStringArray(2020, 14);

        long p1 = d.solvePartOne(program);
        System.out.println("After running the init program the sum of all values in memory is " + p1);
        long p2 = d.solvePartTwo(program);
        System.out.println("After running the init program with the v2 decoder the sum of all values in memory is " + p2);
    }
