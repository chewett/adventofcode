...
public class Day10 {
    ...
    /**
     * Determine how many one and three jolt differences there are and then multiple them together
     * @param jolts List of jolt adapters
     * @return The multiplication of the number of one and three jolt differences
     */
    public long solvePartOne(List<Integer> jolts) {
        //Sorting this will let us judge the gaps between each adapter if strung together
        Collections.sort(jolts);

        int oneJoltDiff = 0;
        //My device accepts +3 jolts so thats one to start with
        int threeJoltDiff = 1;

        int curJolt = 0;
        //Loop over them and log whether the gap is one or three
        for(int joltValue : jolts) {
            long diff = joltValue - curJolt;
    ...
    /**
     * Calculate the different ways we can organise the jolt adapters.
     * @param jolts List of Jolt adapters we have access to
     * @return Number of ways we can organise the jolt adapters
     */
    public long solvePartTwo(List<Integer> jolts) {
        //First sort it and add the voltage of the device to the end (+3 more than the max)
        Collections.sort(jolts);
        jolts.add(jolts.get(jolts.size() - 1) + 3);

        List<List<Integer>> splitSubgroups = new ArrayList<>();

        List<Integer> currentList = new ArrayList<>();
        currentList.add(0);
        int prevNumber = 0;
        //First loop over the adaptor jolts and split the adapters into groups that have multiple ways to connect them
        //You are able to do this as any gap of +3 means that adaptor must be "fixed" in place and always used.
        //By splitting them into subgroups we can calculate the combinations of the subgroups and multiply
        //them together to get the overall combinations. This saves testing every combination!
        for(int i = 0; i < jolts.size(); i++) {
            int diff = jolts.get(i) - prevNumber;
            currentList.add(jolts.get(i));
            if(diff == 3) {
                splitSubgroups.add(currentList);
                currentList = new ArrayList<>();
                currentList.add(jolts.get(i));
            }

            prevNumber = jolts.get(i);
        }

        //Right, we have all the subgroups, now we can calculate the combinations of each subgroup.
        long combinations = 1;
        for(List<Integer> subgroup : splitSubgroups) {
            List<Integer> startingList = new ArrayList<>();
            startingList.add(subgroup.get(0));

            //Create a list of the "middle" elements that we want to create all combinations for
            List<Integer> combsToIterate = new ArrayList<>();
            for(int subgroupIndex=1; subgroupIndex < subgroup.size() -1; subgroupIndex++) {
                combsToIterate.add(subgroup.get(subgroupIndex));
            }

            List<List<Integer>> allCombs = CombinationGenerator.createCombinationsWhereEachElementCanOrCannotExist(combsToIterate);

            //Now add the first and last element to the combinations, this lets us validate each list
            for(List<Integer> listToAddFinalNumberTo : allCombs) {
                listToAddFinalNumberTo.add(0, subgroup.get(0));
                listToAddFinalNumberTo.add(subgroup.get(subgroup.size() - 1));
            }

            //Now lets see which of the combinations are actually valid. Anything jumping more than 3 is invalid
            int foundCombinations = 0;
            for(List<Integer> listToCheck : allCombs) {
                boolean foundToBeValid = true;
                for(int listCheckIndex = 1; listCheckIndex < listToCheck.size(); listCheckIndex++) {
                    int diff = listToCheck.get(listCheckIndex) - listToCheck.get(listCheckIndex -1);
                    if(diff > 3) {
                        foundToBeValid = false;
                        break;
                    }
                }
                if(foundToBeValid) {
    ...
    public static void main(String[] args) {
        ...
        long threeJoltDifferences = d.solvePartOne(jolts);
        System.out.println("The multiplied amount of three jolt distances is " + threeJoltDifferences);

        long p2 = d.solvePartTwo(jolts);
