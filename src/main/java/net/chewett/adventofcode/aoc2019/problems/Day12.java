...
/**
 * Awesome problem taken from: https://adventofcode.com/2019/day/12
 ...
 * a x position of 5, then Ganymede's x velocity changes by +1 (because 5 > 3) and Callisto's x velocity changes by
 * -1 (because 3 < 5). However, if the positions on a given axis are the same, the velocity on that axis does not
 ...
 * energy multiplied by its kinetic energy. A moon's potential energy is the sum of the absolute values of its
 * x, y, and z position coordinates. A moon's kinetic energy is the sum of the absolute values of its velocity
...
public class Day12 {
    ...
    /**
     * Store the permutations of the four moons against each other
     */
    public static int[][] moonPermutations = new int[][] {
            new int[] { 0, 1 },
            new int[] { 0, 2 },
            new int[] { 0, 3 },
            new int[] { 1, 2 },
    ...
    /**
     * Helper function to represent a moons X position and velocity
     * @param moons List of moons
     * @return String representing all the moons X position and velocity
     */
    public String getXString(List<Moon> moons) {
        String s = "";
        for(Moon m : moons) {
    ...
    /**
     * Helper function to represent a moons Y position and velocity
     * @param moons List of moons
     * @return String representing all the moons Y position and velocity
     */
    public String getYString(List<Moon> moons) {
        String s = "";
        for(Moon m : moons) {
    ...
    /**
     * Helper function to represent a moons Z position and velocity
     * @param moons List of moons
     * @return String representing all the moons Z position and velocity
     */
    public String getZString(List<Moon> moons) {
        String s = "";
        for(Moon m : moons) {
    ...
    /**
     * Helper function to convert the input to a moon list
     * @param input String input
     * @return A moon list
     */
    public List<Moon> getStartMoons(List<String> input) {
        List<Moon> moons = new ArrayList<>();
        for(String s : input) {
            String[] parts = s.split("[=,>]");
            moons.add(new Moon(Integer.parseInt(parts[1]), Integer.parseInt(parts[3]), Integer.parseInt(parts[5])));
        }
    ...
    public long solvePartOne(List<String> input) {
        List<Moon> moons = this.getStartMoons(input);

        long steps = 0;
        while(steps < 1000) {
            //Apply gravity for all the moon pairs
            for(int moonPerm = 0; moonPerm < Day12.moonPermutations.length; moonPerm++) {
                int[] moonPair = Day12.moonPermutations[moonPerm];
                moons.get(moonPair[0]).applyGravity(moons.get(moonPair[1]));
            }
            //Then apply velocity for each moon
            for(Moon m : moons) {
                m.applyVelocity();
            }
            steps++;
    ...
    public long solvePartTwo(List<String> input) {
        ...
        String startString = this.getXString(moons);
        String curString;
        long xSteps = 0;
        //Iterate over the X positions to work out when all the moons return to the same (first) state
        while(xSteps < 1000000000) {
            for(int moonPerm = 0; moonPerm < Day12.moonPermutations.length; moonPerm++) {
                int[] moonPair = Day12.moonPermutations[moonPerm];
            ...
            curString = this.getXString(moons);
            if(startString.equals(curString)) {
                break;
            }
        }

        startString = this.getYString(moons);
        long ySteps = 0;
        //Do the same for Y
        while(ySteps < 1000000000) {
            ...
            curString = this.getYString(moons);
            if(startString.equals(curString)) {
                break;
            }
        }

        startString = this.getZString(moons);
        long zSteps = 0;
        //And the same for Z
        while(zSteps < 1000000000) {
            ...
            curString = this.getZString(moons);
            if(startString.equals(curString)) {
                break;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("The total energy of the system is " + partOne);

        long partTwo = d.solvePartTwo(input);
        System.out.println("The number of steps to reach a state we have reached before is " + partTwo);
    }
}
