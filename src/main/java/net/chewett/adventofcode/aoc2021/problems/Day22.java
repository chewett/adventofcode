 ...
 * 3-dimensional coordinate (x,y,z). Each cube can be either on or off; at the start of the reboot process, they are all
...
public class Day22 {
    ...
    public long solvePartOne(List<String> rules) {
        ...
        for(String line : rules) {
            ...
            int xStart = Integer.parseInt(parts[2]);
            int xEnd = Integer.parseInt(parts[3]);
            int yStart = Integer.parseInt(parts[5]);
            int yEnd = Integer.parseInt(parts[6]);
            int zStart = Integer.parseInt(parts[8]);
            int zEnd = Integer.parseInt(parts[9]);

            if(xStart < -50 || xStart > 50 || xEnd > 50 || xEnd < -50 ||
                    yStart < -50 || yStart > 50 || yEnd > 50 || yEnd < -50 ||
                    zStart < -50 || zStart > 50 || zEnd > 50 || zEnd < -50) {
                continue;
            }
            ...
            for(int x = xStart; x <= xEnd; x++) {
                for(int y = yStart; y <= yEnd; y++) {
                    for(int z = zStart; z <= zEnd; z++) {
                        grid.setValueAtPosition(x, y, z, valueTOSet);
                    }
                }
            }
        }
    ...
    public long solvePartTwo(List<String> rules) {
        ...
        long containedArea = 0;
        for(Cuboid c : litCubes) {
            containedArea += c.getContainedArea();
        }
