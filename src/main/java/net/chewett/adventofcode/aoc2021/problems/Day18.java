...
import net.chewett.adventofcode.aoc2021.SnailfishNumber;
import net.chewett.adventofcode.aoc2021.SnailfishParent;
import net.chewett.adventofcode.aoc2021.SnailfishVal;
 ...
 *
 * Add up all of the snailfish numbers from the homework assignment in the order they appear. What is the magnitude
 * of the final sum?
 *
 * --- Part Two ---
...
public class Day18 {
    ...
    /**
     * Takes all the snailfish numbers and adds them together and find the magnitude of the final sum
     * @param sums List of snailfish numbers
     * @return Magnitude of the final sum
     */
    public long solvePartOne(List<String> sums) {
        SnailfishNumber sn = new SnailfishNumber(sums.get(0));

        //Simply sum them up and work out the magnitude
        for(int i = 1; i < sums.size(); i++) {
            SnailfishNumber add = new SnailfishNumber(sums.get(i));
            SnailfishParent p = new SnailfishParent(sn.add(add).toString());
            p.reduce();
            sn = p.getRoot();
        }
    ...
    /**
     * Goes through all the snailfish numbers trying each parent one by one to try and work out the highest magnitude of
     * the sum of two snailfish numbers
     * @param sums List of snailfish numbers
     * @return Largest magnitude of the sum of any two snailfish numbers
     */
    public long solvePartTwo(List<String> sums) {
        int maxMagnitude = 0;
        for(int x = 0; x < sums.size(); x++) {
            for(int y = 0; y < sums.size(); y++) {
                //Ignore when we are comparing the same one
                if(y == x) {
                    continue;
                }
                SnailfishNumber n1 = new SnailfishNumber(sums.get(x));
                SnailfishNumber n2 = new SnailfishNumber(sums.get(y));

                //Try both ways since they may produce different numbers
                SnailfishParent p1 = new SnailfishParent(n1.add(n2).toString());
                p1.reduce();
                int num1 = p1.getRoot().getMagnitude();
                if(num1 > maxMagnitude) {
                    maxMagnitude = num1;
                }

                SnailfishParent p2 = new SnailfishParent(n2.add(n1).toString());
                p2.reduce();
                int num2 = p1.getRoot().getMagnitude();
                if(num2 > maxMagnitude) {
                    maxMagnitude = num2;
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        Day18 d = new Day18();
        System.out.println("THe magnitude of teh final sum is " + d.solvePartOne(sums));
        System.out.println("The largest magnitude of any sum of two different snailfish numbers is " + d.solvePartTwo(sums));
    }
