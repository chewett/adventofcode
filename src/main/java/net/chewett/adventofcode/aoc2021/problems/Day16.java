...
import net.chewett.adventofcode.aoc2021.Packet;
 ...
 * with the most significant bit first. For example, a version encoded as the binary sequence 100 represents the
 * number 4.
 *
...
public class Day16 {
    ...
    /**
     * Solve part 1 to work out the sum of all versions
     * @param packets String representing the packets
     * @return The sum of all the versions in the packets
     */
    public long solvePartOne(String packets) {
        String fullPacketString = ByteHexConversion.convertHexStringToBinaryString(packets);
        Packet rootPacket = new Packet(fullPacketString);

        return rootPacket.getSumOfAllVersions();
    }

    /**
     * Solve Part two and calculate the value of all the packets
     * @param packets String representing the packets
     * @return The value of the packets
     */
    public long solvePartTwo(String packets) {
        String fullPacketString = ByteHexConversion.convertHexStringToBinaryString(packets);
        Packet rootPacket = new Packet(fullPacketString);

        return rootPacket.getValueOfPacket();
    }

    public static void main(String[] args) {
        String packets = ProblemLoader.loadProblemIntoString(2021, 16);

        Day16 d = new Day16();
        long partOne = d.solvePartOne(packets);
        System.out.println("The value of all the version numbers is " + partOne);

        long partTwo = d.solvePartTwo(packets);
        System.out.println("The answer to the expression in the packets is " + partTwo);
    }
