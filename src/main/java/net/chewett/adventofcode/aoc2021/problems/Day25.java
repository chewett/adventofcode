...
public class Day25 {
    ...
    /**
     * Relatively simple permutation solver today
     * @param grid The grid of data
     * @return The number of steps to wait until the sea cucumbers have stopped
     */
    public long solvePartOne(Discrete2DPositionGrid<Character> grid) {

        //Simple while loop to keep running til its finished
        ...
        while(somethingMoved) {
            ...
            //Loop over all the east movers, and move them if they can
            List<Point> allEast = grid.getPositionsOfValue('>');
            for(Point eastCucumber : allEast) {

                Point newEastCucumber = new Point(eastCucumber);
                newEastCucumber.x += 1;
                if(newEastCucumber.x > grid.getMaxX()) {
                    newEastCucumber.x = 0;
                }

                //If there is a space there now we can move into it
                if(grid.getValueAtPosition(newEastCucumber) == '.') {
                    newGrid.setValueAtPosition(newEastCucumber, '>');
                    somethingMoved = true;
                }else{
                    newGrid.setValueAtPosition(eastCucumber, '>');
                }
            }

            //Same again but for south cucumbers
            List<Point> allSouth = grid.getPositionsOfValue('v');
            for(Point southCucumber : allSouth) {

                Point newSouthCucumber = new Point(southCucumber);
                newSouthCucumber.y += 1;
                if(newSouthCucumber.y > grid.getMaxY()) {
                    newSouthCucumber.y = 0;
                }

                //We need to check both the current grid for v's and the new grid for dots, but thats the only trick here
                if(grid.getValueAtPosition(newSouthCucumber) != 'v' && newGrid.getValueAtPosition(newSouthCucumber) == '.') {
                    newGrid.setValueAtPosition(newSouthCucumber, 'v');
                    somethingMoved = true;
                }else{
                    newGrid.setValueAtPosition(southCucumber, 'v');
                }
            }
