...
public class Day4 {
    ...
    /**
     * Work out the value of all the cards that the elves have
     * @param input List of cards
     * @return Total value of all the cards
     */
    public long solvePartOne(List<String> input) {
        ...
        for(String str : input) {
            //Various parsing!
            String[] gameNumbersSplit = str.split(": ");
            String[] numbersSplit = gameNumbersSplit[1].strip().split(" \\| ");
            String[] winningNumbers = numbersSplit[0].strip().split("  ?");
            String[] myNumbersSplit = numbersSplit[1].strip().split("  ?");

            Set<Integer> winningNumbersSet = new HashSet<>();
            for(String winning : winningNumbers) {
                winningNumbersSet.add(Integer.parseInt(winning));
            }

            //Work out the score of eachcard
            int score = 0;
            for(String my : myNumbersSplit) {
                int actualNum = Integer.parseInt(my);

                //Score starts at 1 then doubles each time
                if(winningNumbersSet.contains(actualNum)) {
                    if(score == 0) {
                        score = 1;
                    }else{
                        score *= 2;
    ...
    /**
     * Work out the number of cards I win
     * @param input List of cards
     * @return Total number of all cards
     */
    public long solvePartTwo(List<String> input) {
        //Store the number of cards we have
        Map<Integer, Long> myCards = new HashMap<>();
        long totalCards = 0;
        for(String str : input) {
            //Fun parsing again!
            String[] gameNumbersSplit = str.split(": ");
            ...
            int curCardNo = Integer.parseInt(gameNumbersSplit[0].split("Card")[1].strip());
            //Put the number of cards in at 1 if there is no card marked
            myCards.putIfAbsent(curCardNo, 1L);
            //Then handle the number of cards we have as a multiplier
            long myMult = myCards.get(curCardNo);
            totalCards += myMult;
            ...
            //Then work out what cards these winning cards let you win
            int nextCard = curCardNo + 1;
            for(String my : myNumbersSplit) {
                int actualNum = Integer.parseInt(my);

                if(winningNumbersSet.contains(actualNum)) {
                    long currentCards = myCards.getOrDefault(nextCard, 1L);
                    currentCards += myMult;
                    myCards.put(nextCard, currentCards);
    ...
    public static void main(String[] args) {
        ...
        System.out.println("The total score of all the cards" + partOne);

        long partTwo = d.solvePartTwo(input);
        System.out.println("The number of cards once I have worked out how many I win" + partTwo);
    }
}
