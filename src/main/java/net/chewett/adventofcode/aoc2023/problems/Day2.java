...
public class Day2 {
    ...
    /**
     * Process all the games and work out the sum of the valid game numbers
     * @param lines Lines of games
     * @return Sum of all the valid games
     */
    public long solvePartOne(List<String> lines) {
        ...
        for(String line : lines) {
            ...
            int gameNo = Integer.parseInt(gameParts[0].substring(5));
            String[] rounds = gameParts[1].split("; ");
            boolean possible = true;
            for(String round : rounds) {
                String[] discoveredBalls = round.split(", ");
                for(String foundBall : discoveredBalls) {
                    String[] ballParts = foundBall.split(" ");
                    int ballVal = Integer.parseInt(ballParts[0]);
                    String ballType = ballParts[1];
                    if(ballType.equals("red")) {
                        if(ballVal > maxRed) {
                            possible = false;
                        }
                    }else if(ballType.equals("green")) {
                        if(ballVal > maxGreen) {
                            possible = false;
                        }
                        ...
                        throw new RuntimeException("Found different ball type");
                    }
                }

            }
            if(possible) {
                possibleTotal += gameNo;
            }
        }
    ...
    /**
     * Process all the games and work out the total sum of the power of the cubes
     * @param lines Lines of games
     * @return Total sum of the power of the cubes
     */
    public long solvePartTwo(List<String> lines) {
        long cubePowerTotal = 0;
        for(String line : lines) {
            String[] gameParts = line.split(": ");
            String[] rounds = gameParts[1].split("; ");
            ...
            cubePowerTotal += ((long) minRed * minGreen * minBlue);

        }

        return cubePowerTotal;
    }

    public static void main(String[] args) {
        List<String> lines = ProblemLoader.loadProblemIntoStringArray(2023, 2);

        Day2 d = new Day2();
        long partOne = d.solvePartOne(lines);
        System.out.println("The sum of valid games is " + partOne);

        long partTwo = d.solvePartTwo(lines);
        System.out.println("The sum of the power of cubes is " + partTwo);
    }
}
