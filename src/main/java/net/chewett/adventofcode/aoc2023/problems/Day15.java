 ...
 * another mountain - the largest mountain on Lava Island. As you approach the mountain, you find that the light
 * is being collected by the wall of a large facility embedded in the mountainside.
 *
 ...
 * hard hat. The reindeer leads you to a shelf of goggles and hard hats (you quickly find some that fit) and then
 * further into the facility. At one point, you pass a button with a faint snout mark and the label "PUSH FOR HELP".
 ...
 * collects all of the light from outside. As you admire the large assortment of lenses available to further focus
...
public class Day15 {
    ...
    /**
     * Given the initialisation string this returns the HASH value
     * @param stringToHash Initialisation string to HASH
     * @return HASH value of the string
     */
    private int getHash(String stringToHash) {
        int currentVal = 0;
        for(int i = 0; i < stringToHash.length(); i++) {
            char curChar = stringToHash.charAt(i);
            currentVal += curChar;
            currentVal *= 17;
    ...
    /**
     * Loop over each part of the string and calculate the hash values and then sum then up
     * @param input String of initialisation values
     * @return Total of the initialisation values
     */
    ...
    /**
     * Runs the HASHMAP algorithm to find out the various positions of the lenses and then work out the focussing
     * power of all the lenses
     * @param input String of initialisation values
     * @return Sum of all the focussing power
     */
    public long solvePartTwo(String input) {
        ...
        for(String s : steps) {
            ...
            int hashVal = this.getHash(parts[0]);

            if(!boxes.containsKey(hashVal)) {
                boxes.put(hashVal, new LinkedList<>());
            }

            //If there are two parts then it's an equals with a lens value
            if(parts.length == 2) {
                int lensVal = Integer.parseInt(parts[1]);
                int indexOfdata = boxes.get(hashVal).indexOf(parts[0]);
                //lens label already in the box so we need to replace the value
                if(indexOfdata != -1) {
                    //Remove the old lens
                    boxes.get(hashVal).remove(indexOfdata+1);
                    //Re-add the new lens
                    boxes.get(hashVal).add(indexOfdata+1, ""+lensVal);
                }else{
                    //Otherwise just add it
                    boxes.get(hashVal).add(parts[0]);
                    boxes.get(hashVal).add(""+lensVal);
                }

            //Otherwise it's a dash
            }else{
                //If we find the index, pop both parts out
                int indexOfdata = boxes.get(hashVal).indexOf(parts[0]);
                if(indexOfdata != -1) {
                    boxes.get(hashVal).remove(indexOfdata);
                    boxes.get(hashVal).remove(indexOfdata);
                }
            }
        }

        //Once we have performed all the lens manipulation we can then work out the focussing power
        long total = 0;
        for(Map.Entry<Integer, List<String>> e : boxes.entrySet()) {
            int boxPower = e.getKey() + 1;
            for(int i = 0; i < e.getValue().size(); i+=2) {
                int slotNo = (i / 2)+1;
                total += boxPower * slotNo * Integer.parseInt(e.getValue().get(i+1));
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("The sum of all the hashes " + partOne);

        long partTwo = d.solvePartTwo(input);
        System.out.println("The sum of all the focussing power " + partTwo);
    }
}
