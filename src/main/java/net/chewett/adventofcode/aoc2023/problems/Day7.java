...
import net.chewett.adventofcode.aoc2023.Day7Hand;
 ...
 * Island Island, making it hard to even get there. Normally, they use big machines to move the rocks and filter the
 * sand, but the machines have broken down because Desert Island recently stopped receiving the parts they need to fix
 * the machines.
 *
...
public class Day7 {
    ...
    /**
     * Find out the total winnings by ordering the cards and multiplying the rank by the bid
     * @param input List of cards and bids
     * @return Total winnings
     */
    public long solvePartOne(List<String> input) {
        List<Day7Hand> hands = new ArrayList<>();
        for(String line : input) {
            hands.add(new Day7Hand(line));
        }

        //Use the comparator in the class and order it nicely :)
        Collections.sort(hands);

        //Do some maths to work out the total winnings!
        long totalWinnings = 0;
        for(int rank = 0; rank < hands.size(); rank++) {
            totalWinnings += (rank+1) * hands.get(rank).getBid();
        }
    ...
    public long solvePartTwo(List<String> input) {
        List<Day7Hand> hands = new ArrayList<>();
        for(String line : input) {
            hands.add(new Day7Hand(line, true));
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("The total winnings without playing with jokers is " + partOne);

        long partTwo = d.solvePartTwo(input);
        System.out.println("The total winners playing with jokers is " + partTwo);
    }
}
