 ...
 * column. For keys, the columns make up the shape of the key where it aligns with pins; those can also be converted
 * to a list of heights."
 *
...
public class Day25 {
    ...
    /**
     * Create the locks and keys into a 2d grid, then convert them into the number form and check they don't overlap,
     * counting ones that don't overlap
     * @param input Keys and locks
     * @return Number of locks that don't overlap
     */
    public long solvePartOne(List<String> input) {
        List<Discrete2DPositionGrid<Character>> grids = new ArrayList<>();

        //Create a series of grids representing locks and keys
            ...
            if(line.equals("")) {
                grids.add(
                    FormatConversion.convertCharArrayIntoDiscrete2DPositionGridCharacter(
                        FormatConversion.convertStringArrayToCharListList(curGrid)
                    )
                );
        ...
        for(Discrete2DPositionGrid<Character> grid : grids) {
            //Work out if it is a lock or a key
            boolean lock = false;
            if(grid.getValueAtPosition(0, 0) == '#') {
                lock = true;
            }

            //Then work out the profile for each
            List<Integer> profile = new ArrayList<>();
            if(lock) {
                for (int x = 0; x <= grid.getWidth(); x++) {
                    for(int y = 1; y <= grid.getHeight(); y++) {
                        if(grid.getValueAtPosition(x, y) == '.') {
                            profile.add(y-1);
                            break;
                        }
                    }
                ...
                for (int x = 0; x <= grid.getWidth(); x++) {
                    for(int y = grid.getHeight()-1; y >= 0; y--) {
                        if(grid.getValueAtPosition(x, y) == '.') {
                            profile.add(grid.getHeight() - y - 2);
                            break;
                        }
                    }
                }
        ...
        //Loop over all keys and locks summing their size, if they exceed 5 then they overlap in some form
        long fits = 0;
    ...
    public static void main(String[] args) {
        ...
        System.out.println("The number of non-overlapping keys and locks are " + partOne);
    }
}
